...
int main()
{
...
#if 1
    Mat color_out = Mat::zeros( depth.rows, depth.cols, CV_8UC3 );
    //Mat depth_out = Mat::zeros( color.rows, color.cols, CV_16UC1 );
    Mat multi1_out = Mat::zeros( depth.rows, depth.cols, CV_16UC1 );
    Mat inf1_out = Mat::zeros( depth.rows, depth.cols, CV_8UC1 );
    Mat inf2_out = Mat::zeros( depth.rows, depth.cols, CV_8UC1 );
    Mat heat_out = Mat::zeros( depth.rows, depth.cols, CV_8UC3 );
    for(int m=0;m<depth.rows;m++)
    {
        for(int n=0;n<depth.cols;n++)
        {
            ...
            u_ = multi1PixPoint[0]/multi1PixPoint[2];
            v_ = multi1PixPoint[1]/multi1PixPoint[2];
            if( u_<0||u_>=multi1.cols || v_<0||v_>=multi1.rows ){}
            else
                multi1_out.ptr<ushort>(m)[n] = multi1.ptr<ushort>(v_)[u_];

            //inf1对齐到depth
            Eigen::Vector3f inf1PixPoint = CameraInf1 * (T_depth_inf1.block(0,0,3,3) * depthFramPoint + T_depth_inf1.block(0,3,3,1));
            u_ = inf1PixPoint[0]/inf1PixPoint[2];
            v_ = inf1PixPoint[1]/inf1PixPoint[2];
            if( u_<0||u_>=inf1.cols || v_<0||v_>=inf1.rows ){}
            else  
                inf1_out.ptr<uchar>(m)[n] = inf1.ptr<uchar>(v_)[u_];
            
            //inf2对齐到depth
            Eigen::Vector3f inf2PixPoint = CameraInf2 * (T_depth_inf2.block(0,0,3,3) * depthFramPoint + T_depth_inf2.block(0,3,3,1));
            u_ = inf2PixPoint[0]/inf2PixPoint[2];
            v_ = inf2PixPoint[1]/inf2PixPoint[2];
            if( u_<0||u_>=inf2.cols || v_<0||v_>=inf2.rows ){}
            else  
                inf2_out.ptr<uchar>(m)[n] = inf2.ptr<uchar>(v_)[u_];
            
            //heat对齐到depth
            Eigen::Vector3f heatPixPoint = CameraHeat * (T_depth_heat.block(0,0,3,3) * depthFramPoint + T_depth_heat.block(0,3,3,1));
            u_ = heatPixPoint[0]/heatPixPoint[2];
            v_ = heatPixPoint[1]/heatPixPoint[2];
            if( u_<0||u_>=heat.cols || v_<0||v_>=heat.rows ){}
            else  
            {
                heat_out.ptr<uchar>(m)[n*3+1] = heat.ptr<uchar>(v_)[u_*3+1];  //b
                heat_out.ptr<uchar>(m)[n*3+2] = heat.ptr<uchar>(v_)[u_*3+2];  //g
                heat_out.ptr<uchar>(m)[n*3+3] = heat.ptr<uchar>(v_)[u_*3+3];  //r
            }
        }
    }

    imwrite("../result/color_out.png", color_out);  
    imwrite("../result/depth.png", depth); 
    imwrite("../result/color.png", color);
    //imwrite("../result/heatsrc.png", heat);
